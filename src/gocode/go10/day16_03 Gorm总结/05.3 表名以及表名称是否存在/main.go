...
func main() {
	...
	//判断表是否存在  db.Migrator().HasTable(&Teacher{}) 返回bool值, true或者false
	if db.Migrator().HasTable(&Teacher{}) == true {
		fmt.Println("表存在")
	}

	//判断表名是否存在,和上边一样也是bool值
	if db.Migrator().HasTable("teachers") == true {
		fmt.Println("表名存在")
	}
}
